...
async function getWalmartCookie(zip) {
    ...
    let locationData = '';
    const browser = await puppeteer.launch({ headless: true, defaultViewport: null, args: ['--no-sandbox', '--disable-setuid-sandbox'], executablePath: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'});
    ...
    try {
        ...
        console.log(`Failed on ${zip} for walmart`);
    } finally {
        await page.close();
        await browser.close();
    }
...
async function getTargetCookie(zip){
    ...
    try {
        ...
        console.log(`Failed on ${zip} for target`);
        console.log(err)
    } finally {
        await page.close();
        await browser.close();
